...
bool db_config_artificial_table_fdb_backend_t::write_row(
    ...
    if (new_value_inout->has()) {
        ...
        if (existed_before) {
            ...
            // We do pass back a shared task id when dropping a db via the db drop
            // command, but not here of course.
            fdb_shared_task_id task_id = config_cache_db_drop_uuid(txn, user_context, database_id, old_db_name,
                interruptor);
            (void)task_id;
        }
        return true;
