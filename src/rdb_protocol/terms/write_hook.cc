        ...
        try {
            fdb_error_t loop_err = txn_retry_loop_coro(env->env->get_rdb_ctx()->fdb, env->env->interruptor, [&](FDBTransaction *txn) {
                ...
                std::pair<namespace_id_t, table_config_t> info
                    = expect_retrieve_table(txn, table, env->env->interruptor).ci_value;

                // We could save a round trip -- the table_config_t fut in
                // expect_retrieve_table and the auth fut here could be done
                // simultaneously.
                ...
                auth::fdb_user_fut<auth::db_table_config_permission> auth_fut
                    = env->env->get_user_context().transaction_require_db_and_table_config_permission(
                        txn, info.second.basic.database, info.first);
